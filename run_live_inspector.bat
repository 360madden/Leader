...
if not exist "debug" (
    ...
    if errorlevel 1 (
        set "EXIT_CODE=1"
        call "%ROOT%log_launcher_failure.bat" "%ROOT%" "%LAUNCHER%" "mkdir_debug" "1" "Failed to create the debug folder in LeaderLiveInspector."
        echo [ERROR] Could not create the LeaderLiveInspector debug folder.
        goto :finish
    )
)
